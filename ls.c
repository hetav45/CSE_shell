...
void exec_ls(char * arg) {
    ...
    char dir_path[1024], * temp, * flag, time[64], file_path[1024];

    dir_path[0] = '.';  dir_path[1] = '\0';   // default

    if(arg != NULL) {
        temp = arg;
        while((flag = strtok_r(temp, " \t", &temp))) {
            if(strcmp(flag, "-l") == 0) flag_l = 1;
            else if(strcmp(flag, "-a") == 0) flag_a = 1;
            else if(strcmp(flag, "-la") == 0) { flag_l = 1; flag_a = 1; }
            else if(strcmp(flag, "-al") == 0) { flag_l = 1; flag_a = 1; }
            else strcpy(dir_path, flag);
        }
    }
        ...
        if(flag_l) {  // man 7 inode
            ...
            if( (dir_stat->d_name[0] == '.' && flag_a) || dir_stat->d_name[0] != '.' ) {
                ...
                switch (file_stat.st_mode & S_IFMT) {
                    case S_IFDIR : printf("d"); break;  // dir
