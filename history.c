...
void insert_history(char * arg) {
    ...
    if(strcmp(record[(end_ptr-1) % r_sz], arg)  == 0) return; 

    strncpy(record[end_ptr], arg, 1023);

    end_ptr = (end_ptr + 1) % r_sz;

    if(end_ptr == start_ptr) start_ptr = (start_ptr + 1) % r_sz;
}
...
void retrieve_history() {
    ...
    for(int i = 0; i < (r_sz*1025); i++) buf[i] = 0;  // read wont null terminate
